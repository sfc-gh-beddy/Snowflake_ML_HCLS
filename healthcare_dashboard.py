...
try:
    ...
    from snowflake.snowpark.functions import col, avg, count, max as max_, min as min_
...
if not inference_table_exists and not batch_table_exists:
    ...
    try:
        ...
        if inference_table_exists:
            try:
                inference_data = session.table("ADVERSE_EVENT_MONITORING.DEMO_ANALYTICS.INFERENCE_REQUEST_LOG").limit(100).to_pandas()
        ...
        if batch_table_exists:
            try:
                population_data = session.table("ADVERSE_EVENT_MONITORING.DEMO_ANALYTICS.BATCH_INFERENCE_RESULTS").limit(1000).to_pandas()
...
try:
    ...
    logs_query = """
        SELECT * FROM ADVERSE_EVENT_MONITORING.DEMO_ANALYTICS.INFERENCE_REQUEST_LOG
